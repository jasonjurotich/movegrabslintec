...
pub async fn get_all_org_group_mappings(
  abr: String,
) -> AppResult<HashMap<String, String>> {
  let ep = Ep::Orgbase;

  let qry =
    "select data.org, data.grupo from type::table($table) where abr = $abr;";
...
pub async fn get_grp_from_orgbase(
...

  {
    ...
    if let Some(mappings) = cache.get(&abr) {
      if let Some(grupo) = mappings.get(&org) {
        debug!(
          "get_grp_from_orgbase cache hit for abr={}, org={}: {}",
          abr, org, grupo
        );
        return Ok(grupo.clone());
      } else {
        debug!(
          "get_grp_from_orgbase cache hit for abr={} but org '{}' not found in {} mappings",
          abr,
          org,
          mappings.len()
        );
  ...
  debug!(
    "get_grp_from_orgbase cache miss for abr={}, org={}, falling back to database",
    abr, org
  );
  ...
  debug!(
    "get_grp_from_orgbase raw result for org '{}': {:?}",
    org, result
  );
